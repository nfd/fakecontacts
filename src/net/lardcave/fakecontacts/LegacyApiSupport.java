...
import net.lardcave.fakecontacts.ContactsDatabaseHelper.ExtensionsColumns;
...
public class LegacyApiSupport {
    ...
    static {
        ...
        String authority = FakeContract.LEGACY_AUTHORITY;
...
//        matcher.addURI(authority, "voice_dialer_timestamp", VOICE_DIALER_TIMESTAMP);
    ...
    public LegacyApiSupport(Context context, ContactsDatabaseHelper contactsDatabaseHelper,
        ...
        mPhoneticNameSplitter = new NameSplitter("", "", "", context
                .getString(R.string.common_name_conjunctions), Locale
                .getDefault());
    ...
    private long insertPeople(ContentValues values) {
        ...
        Uri contactUri = mContactsProvider.insertInTransaction(FakeContract.RAW_CONTACTS_CONTENT_URI, mValues);
        long rawContactId = ContentUris.parseId(contactUri);

        if (mValues2.size() != 0) {
            mValues2.put(Data.RAW_CONTACT_ID, rawContactId);
            mContactsProvider.insertInTransaction(FakeContract.DATA_CONTENT_URI, mValues2);
        }
        if (mValues3.size() != 0) {
            mValues3.put(Data.RAW_CONTACT_ID, rawContactId);
            mContactsProvider.insertInTransaction(FakeContract.DATA_CONTENT_URI, mValues3);
        }
    ...
    private long insertOrganization(ContentValues values) {
        ...
        Uri uri = mContactsProvider.insertInTransaction(FakeContract.DATA_CONTENT_URI, mValues);
    ...
    private long insertGroup(ContentValues values) {
        ...
        Uri uri = mContactsProvider.insertInTransaction(FakeContract.GROUPS_CONTENT_URI, mValues);
        return ContentUris.parseId(uri);
    }
    ...
    private int updatePeople(long rawContactId, ContentValues values) {
        ...
        int count = mContactsProvider.update(FakeContract.RAW_CONTACTS_CONTENT_URI,
    ...
    private int updateOrganizations(long dataId, ContentValues values) {
        ...
        return mContactsProvider.updateInTransaction(FakeContract.DATA_CONTENT_URI, mValues,
    ...
    private int updateGroups(long groupId, ContentValues values) {
        ...
        return mContactsProvider.updateInTransaction(FakeContract.GROUPS_CONTENT_URI, mValues,
    ...
    private int updatePhoto(long rawContactId, ContentValues values) {
        ...
        if (dataId == -1) {
            ...
            mValues.put(Data.RAW_CONTACT_ID, rawContactId);
            Uri dataUri = mContactsProvider.insertInTransaction(FakeContract.DATA_CONTENT_URI, mValues);
            dataId = ContentUris.parseId(dataUri);
            count = 1;
        } else {
            Uri dataUri = ContentUris.withAppendedId(FakeContract.DATA_CONTENT_URI, dataId);
    ...
    private int updatePhotoByDataId(long dataId, ContentValues values) {
        ...
        if (values.containsKey(android.provider.Contacts.Photos.DATA)) {
            ...
            mValues.clear();
            mValues.put(Photo.PHOTO, bytes);
            mContactsProvider.updateInTransaction(FakeContract.DATA_CONTENT_URI, mValues,
                    Data._ID + "=" + dataId, null);
        }
    ...
    private void updateLegacyPhotoData(long rawContactId, long dataId, ContentValues values) {
        ...
        int updated = mContactsProvider.updateInTransaction(FakeContract.DATA_CONTENT_URI, mValues,
        ...
        if (updated == 0) {
            ...
            mContactsProvider.insertInTransaction(FakeContract.DATA_CONTENT_URI, mValues);
        }
    }
    ...
    private int updateSettings(ContentValues values) {
        ...
        if (key.equals(android.provider.Contacts.Settings.SYNC_EVERYTHING)) {
            ...
            int count = mContactsProvider.updateInTransaction(FakeContract.SETTINGS_CONTENT_URI, mValues,
                    selection, selectionArgs);
            if (count == 0) {
                mValues.put(Settings.ACCOUNT_NAME, accountName);
                mValues.put(Settings.ACCOUNT_TYPE, accountType);
                mContactsProvider.insertInTransaction(FakeContract.SETTINGS_CONTENT_URI, mValues);
            }
        }
        return 1;
    }
    ...
    private Uri findFirstDataRow(long rawContactId, String contentItemType) {
        ...
        return ContentUris.withAppendedId(FakeContract.DATA_CONTENT_URI, dataId);
    }

    private long findFirstDataId(long rawContactId, String mimeType) {
        long dataId = -1;
        Cursor c = mContactsProvider.query(FakeContract.DATA_CONTENT_URI, IdQuery.COLUMNS,
    ...
    public Cursor query(Uri uri, String[] projection, String selection, String[] selectionArgs,
        ...
        if (c != null) {
            c.setNotificationUri(mContext.getContentResolver(), FakeContract.LEGACY_CONTENT_URI);
        }
        return c;
    }
    ...
    private void onChange(Uri uri) {
        mContext.getContentResolver().notifyChange(FakeContract.LEGACY_CONTENT_URI, null);
    }
